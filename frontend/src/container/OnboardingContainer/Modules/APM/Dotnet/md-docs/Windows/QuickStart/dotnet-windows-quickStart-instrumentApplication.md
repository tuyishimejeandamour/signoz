...
**Step 4: Generating some load data and checking your application in Trinity UI**

Once your application is running, generate some traffic by interacting with it.

In the Trinity account, open the `Services` tab. Hit the `Refresh` button on the top right corner, and your application should appear in the list of `Applications`. Ensure that you're checking data for the `time range filter` applied in the top right corner. You might have to wait for a few seconds before the data appears on Trinity UI.