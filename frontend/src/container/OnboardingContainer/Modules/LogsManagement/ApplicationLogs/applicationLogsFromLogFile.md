## Collect Application Logs from Log file in Trinity cloud
...
- Add the filelog reciever to `config.yaml`.
  ...
  Note: change the `start_at` value to `beginning` if you want to read the log file from the beginning. It may be useful if you want to send old logs to Trinity. The log records older than the standard log retention period (default 15 days) will be discarded.
...
- If there are no errors your logs will be visible on Trinity UI.
