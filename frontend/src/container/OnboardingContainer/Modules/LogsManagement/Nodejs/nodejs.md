...
If you are using `winston` as your logging library in your Nodejs application, you can export these logs to Trinity very easily using various transports provided by `winston`.

## Collecting Nodejs logs when application is deployed on Docker or Kubernetes

When your application is deployed in Docker or a Kubernetes cluster the logs from the console are automatically collected and stored in the node. The Trinity collector will automatically collect the logs and it will be visible on the Trinity UI.
